...
const book = reactive({
    name: '美国小学学地道口语', pages: [
        { page: 0, buttons: [{ boundingPolygon: [{ x: 100, y: 100 }, { x: 200, y: 100 }, { x: 200, y: 200 }, { x: 100, y: 200 }], width: 30, height: 30, text: '你好，世界！', audio: '' }] }
    ]
})
const page = reactive({})
...
const getFramedText = () => {
    ...
    page.coordinates.readResults[0].lines.forEach((item, index) => {
        ...
        // }
        if ((item.boundingBox[0] * ratio.value).toFixed(2) >= SelectRect.value.x().toFixed(2)
            && (item.boundingBox[1] * ratio.value).toFixed(2) >= SelectRect.value.y().toFixed(2)
            && (item.boundingBox[2] * ratio.value).toFixed(2) <= (parseFloat(SelectRect.value.x().toFixed(2)) + parseFloat(SelectRectEndWidth.value)).toFixed(2)
            && (item.boundingBox[3] * ratio.value).toFixed(2) >= SelectRect.value.y().toFixed(2)

            && (item.boundingBox[4] * ratio.value).toFixed(2) <= (parseFloat(SelectRect.value.x().toFixed(2)) + parseFloat(SelectRectEndWidth.value)).toFixed(2)
            && (item.boundingBox[5] * ratio.value).toFixed(2) <= (parseFloat(SelectRect.value.y().toFixed(2)) + parseFloat(SelectRectEndHeight.value)).toFixed(2)
            && (item.boundingBox[6] * ratio.value).toFixed(2) >= SelectRect.value.x().toFixed(2)
            && (item.boundingBox[7] * ratio.value).toFixed(2) <= (parseFloat(SelectRect.value.y().toFixed(2)) + parseFloat(SelectRectEndHeight.value)).toFixed(2)
        ) {
            text += item.text + ' '
        }
    })
...
const addToPage = async () => {
    if (SelectRect.value) {
        let newContent = {
            boundingPolygon: [
                { x: SelectRect.value.x().toFixed(2) / ratio.value, y: SelectRect.value.y().toFixed(2) / ratio.value },
                { x: (SelectRect.value.x() + SelectRect.value.width()) / ratio.value, y: SelectRect.value.y() / ratio.value },
                { x: (SelectRect.value.x() + SelectRect.value.width()) / ratio.value, y: (SelectRect.value.y() + SelectRect.value.height()) / ratio.value },
                { x: SelectRect.value.x() / ratio.value, y: (SelectRect.value.y() + SelectRect.value.height()) / ratio.value }
            ],
...
const enhanceAudioContentCreate = async () => {
    ...
    let originalTextAudioPromise = new Promise(async (resolve, reject) => {
        try {
            ...
            await edgeTtsTemp.setMetadata(enhanceAudioOption.originalTextvoice, OUTPUT_FORMAT.AUDIO_24KHZ_48KBITRATE_MONO_MP3);
            let options = {
            ...
            readable.on("end", () => {
                ...
                page.content[enhanceAudioSettingItemIndex.value].audio = 'data:audio/mp3;base64,' + originalTextBase64;
                resolve()
            });
            readable.on("closed", () => {
    ...
    let translateTextAudioPromise = new Promise(async (resolve, reject) => {
        try {
            let edgeTtsTemp = new MsEdgeTTS()
            await edgeTtsTemp.setMetadata(enhanceAudioOption.translateTextvoice, OUTPUT_FORMAT.AUDIO_24KHZ_48KBITRATE_MONO_MP3);
            let options = {
            ...
            readable.on("end", () => {
                ...
                page.content[enhanceAudioSettingItemIndex.value].translateAudio = 'data:audio/mp3;base64,' + translateTextBase64;
                resolve()
            });
            readable.on("closed", () => {
                console.log("STREAM CLOSED");
    ...
    // 保存到本地
    await savePageData()
    console.log('-------------enhanceAudioContentCreate-', page.content[enhanceAudioSettingItemIndex.value])    
    // 弹窗提示
    ElMessage({
...
const audioContentCreate = async () => {
    ...
    try {
        ...
        for (let i = 0; i < page.content.length; i++) {
            ...
            primiseAll.push(new Promise(async (resolve, reject) => {
                try {
                    let edgeTtsTemp = new MsEdgeTTS()
                    await edgeTtsTemp.setMetadata(audioOption.currentVoice, OUTPUT_FORMAT.AUDIO_24KHZ_48KBITRATE_MONO_MP3);
                    let options = {
                        volume: audioOption.volume,
                        rate: audioOption.speed,
                        pitch: audioOption.pitch
...
const downloadAudio = async (text) => {
    if (typeof speechSynthesis === "undefined" || text === '' || navigator.mediaDevices === undefined) {
        return;
    }
}
...
async function savePageData() {
    ...
    // 保存page数据到本地存储
    await db.page.update(page.id, { content: JSON.stringify(page.content), image: JSON.stringify(page.image), coordinates: JSON.stringify(page.coordinates) })
    ...
    if (page.coordinates) {
        try {
            page.coordinates.readResults[0].lines.forEach(item => {
                ...
                console.log('item.boundingBox', item.boundingBox[0] * ratio.value, item.boundingBox[1] * ratio.value,
                    (item.boundingBox[3] - item.boundingBox[0]) * ratio.value, (item.boundingBox[5] - item.boundingBox[1]) * ratio.value * 1.2)
                let rectItem = new Konva.Rect({    // 矩形
            ...
            tr.on('transform', function (e) {
                ...
                SetSelectRectInfo(SelectRect.value.x(), SelectRect.value.y(), SelectRect.value.width() * e.target.attrs.scaleX, SelectRect.value.height() * e.target.attrs.scaleY, SelectRect.value.width(), SelectRect.value.height())
            })
    ...
    */
    ...
    if (ocr.servicefrom === 'azure') {
        ...
        axios.post(url, form, { headers: headers }).then(res => {
            ...
            let url2 = import.meta.env.PROD? ocr.azure.endpoint + 'vision/v3.2/read/analyzeResults/' + operationId : '/azure2' + `${operationId}`
            let timer = setInterval(() => {
                ...
                axios.get(url2, { headers: header2 }).then(res => {
                    if (res.data.status === 'succeeded') {
                        clearInterval(timer)
                        console.log('ocrAnalyzeImage res.data.recognitionResult.lines', res.data.analyzeResult)
...
/* modify element plus css class default */
.el-card__header {
...
<template>
    ...
    <div class="position-fixed d-flex justify-content-center align-items-center" style="bottom: 50px; right: 60px;">
        <el-tooltip class="box-item" effect="light"
            content="<span>所有数据仅存储在本地localStorage和IndexedDB中，不会留存至服务器。<br/>仅用于客户端设计和接口数据转发使用。</span>" placement="top"
            raw-content>
    ...
    <div class="h5 position-fixed top-0 start-0 w-auto d-flex justify-content-center align-items-center p-4">
        aoaoms 多媒体音频图片制作工具
    </div>
    <div
        class="container-fluid mt-3 shadow-sm border w-25 py-3 rounded-pill align-center d-flex justify-content-center align-items-center z-3">
        ...
        <span class="mx-3" style="font-size: 14px;color: #999;">图片：{{ imgtW }} x {{ imgtH }}</span>
        <span style="font-size: 14px;color: #999;">画布宽高比：3:4</span>
    </div>
    <div class="d-flex justify-content-center align-items-end" style="height: 30px;">
        <!-- <audio id="idaudio" controls src=""></audio> -->
    </div>
    <div class=" d-flex justify-content-center align-items-center ">
        <div id="idcanvas" class="" :class="cursorClass">
        </div>
    </div>
        ...
        <el-card style="width: 300px;" class="">
            <template #header>
                <div class="card-header d-flex justify-content-around align-items-center">
                    ...
                    <el-tooltip class="box-item" effect="light" content="查看数据" placement="top">
                        <el-button icon="DataBoard" class="p-2" type="" siz="large" text
                            @click="viewPageData"></el-button>
                    </el-tooltip>

                    <el-tooltip class="box-item" effect="light" content="下载设计" placement="top">
                        <el-button icon="Download" class="p-2" type="" siz="large" text
                            @click="downloadPage"></el-button>
                    </el-tooltip>
                    ...
                    <el-tooltip class="box-item" effect="light" content="OCR服务设置" placement="top">
                        <el-button icon="Setting" class="p-2" type="" siz="large" text
                            @click="ocrSettingDialogVisible = true"></el-button>
                    </el-tooltip>
                    <el-dropdown trigger="click">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu class="p-1 d-flex flex-column justify-content-between align-items-center">
            ...
            <div class="overflow-auto  px-0" style="height: 40vh;">
                <div v-for="(item, index) in page.content " :key="index"
                    ...
                    <el-button icon="VideoPlay" class="p-2" type="" siz="small" text
                        @click="playItemAudio(index)"></el-button>
                    <el-text class="w-60px d-block" truncated>
                        <span class="btn text-start m-0 p-0" @click="enhanceAudioSetting(index)">{{ item.text }}</span>                        
                    </el-text>
                    <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" icon="Warning" icon-color="red"
                        ...
                        <template #reference>
                            <div class="float-right end-0 me-2"
            ...
            <template #footer>
                <el-tabs class="demo-tabs">
                    ...
                    <el-tab-pane label="框定信息">
                        <el-input size="small" readonly v-model="SelectRectInfo" :rows="5" type="textarea"
                            placeholder="" />
                    </el-tab-pane>
                </el-tabs>
    ...
    <el-dialog v-model="enhanceAudioDialogVisible" title="增强音频设置" width="45vw" destroy-on-close center>
        <div class="d-flex flex-column justify-content-around align-content-start">
            ...
            <div class="d-flex justify-content-between align-content-start w-100">
                <el-scrollbar height="35vh" class="col-7">
                    ...
                    <p v-for="(item, index) in ttsvoice.filter(item => item.locale.includes(filterVoiceKey))"
                        :key="index" class="btn scrollbar-demo-item text-start"
                        @click="enhanceAudioOption.currentVoice = item.voice; enhanceAudioOption.lang = item.locale;">{{ item.alias }}
                ...
                <div class="text-start col-4 me-2">
                    ...
                    <el-select v-model="enhanceAudioOption.speed" placeholder="Select" class="ms-5" style="width: 120px">
                        <el-option
                            v-for="item in [{ 'label': '默认', 'value': 'default' }, { 'label': '超快', 'value': 'x-fast' }, { 'label': '快速', 'value': 'fast' }, { 'label': '中等', 'value': 'medium' }, { 'label': '慢速', 'value': 'slow' }, { 'label': '超慢', 'value': 'x-slow' }]"
                            :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                    <!-- <el-slider v-model="audioOption.speed" :min="0.5" :max="2" :step="0.1"></el-slider> -->
                    <br><span>音调</span>
                    <el-select v-model="enhanceAudioOption.pitch" placeholder="Select" class="ms-5 my-4" style="width: 120px">
                        <el-option
                            v-for="item in [{ 'label': '默认', 'value': 'default' }, { 'label': '超高', 'value': 'x-high' }, { 'label': '高', 'value': 'high' }, { 'label': '中等', 'value': 'medium' }, { 'label': '底', 'value': 'low' }, { 'label': '超低', 'value': 'x-low' }]"
                            :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                    <!-- <el-slider v-model="audioOption.pitch" :min="0.1" :max="1" :step="0.1"></el-slider> -->
                    <br><span>音量</span>
                    <el-select v-model="enhanceAudioOption.volume" placeholder="Select" class="ms-5 " style="width: 120px">
                        <el-option
                            v-for="item in [{ 'label': '默认', 'value': 'default' }, { 'label': '大声', 'value': 'loud' }, { 'label': '平静', 'value': 'silent' }, { 'label': '中等', 'value': 'medium' }, { 'label': '温柔', 'value': 'soft' }, { 'label': '聒噪', 'value': 'x-loud' }, { 'label': '细语', 'value': 'x-soft' }]"
                            :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                </div>
            </div>
            <div class="my-2 d-flex justify-content-end align-content-center">
                <el-button type="primary" @click="enhanceAudioOption.originalTextvoice = enhanceAudioOption.currentVoice;
                    ...
                    enhanceAudioOption.originalTextVoiceVolume = enhanceAudioOption.volume;">设置源音频播报</el-button>
                <el-button type="primary" @click="enhanceAudioOption.translateTextvoice = enhanceAudioOption.currentVoice;
                    ...
                    enhanceAudioOption.translateTextVoiceVolume = enhanceAudioOption.volume;">设置翻译音频播报</el-button>
            </div>
            <!-- 翻译设置 -->
        ...
        <template #footer>
            <div class="dialog-footer d-flex justify-content-end">
                ...
                <el-button @click="enhanceAudioDialogVisible = false">关闭</el-button>
            </div>
        </template>
    </el-dialog>
    ...
    <el-dialog v-model="audioSettingDialogVisible" title="批量设置音频" width="45vw" destroy-on-close center>
        <div class="d-flex flex-column justify-content-around align-content-start">
            ...
            <div class="d-flex justify-content-between align-content-start w-100">
                <el-scrollbar height="50vh" class="col-7">
                    ...
                    <p v-for="(item, index) in ttsvoice.filter(item => item.locale.includes(filterVoiceKey))"
                        :key="index" class="btn scrollbar-demo-item text-start"
                        @click="audioOption.currentVoice = item.voice; audioOption.lang = item.locale;">{{ item.alias }}
                        <span class="ms-2">{{ item.name }}</span>
                        <span v-if="item.gender === 'Female'" class="ms-2">女士</span>
                        <span v-else-if="item.gender === 'Male'" class="ms-2">男士</span>
                        <span v-else-if="item.gender === 'Female,Child'" class="ms-2">女童</span>
                ...
                <div class="text-start col-4 me-2">
                    ...
                    <div style="height:20px;"></div>
                    <span class="mt-3">速度</span>
                    <el-select v-model="audioOption.speed" placeholder="Select" class="ms-5" style="width: 120px">
                        <el-option
                            v-for="item in [{ 'label': '默认', 'value': 'default' }, { 'label': '超快', 'value': 'x-fast' }, { 'label': '快速', 'value': 'fast' }, { 'label': '中等', 'value': 'medium' }, { 'label': '慢速', 'value': 'slow' }, { 'label': '超慢', 'value': 'x-slow' }]"
                            :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                    <!-- <el-slider v-model="audioOption.speed" :min="0.5" :max="2" :step="0.1"></el-slider> -->
                    <br><span>音调</span>
                    <el-select v-model="audioOption.pitch" placeholder="Select" class="ms-5 my-4" style="width: 120px">
                        <el-option
                            v-for="item in [{ 'label': '默认', 'value': 'default' }, { 'label': '超高', 'value': 'x-high' }, { 'label': '高', 'value': 'high' }, { 'label': '中等', 'value': 'medium' }, { 'label': '底', 'value': 'low' }, { 'label': '超低', 'value': 'x-low' }]"
                            :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                    <!-- <el-slider v-model="audioOption.pitch" :min="0.1" :max="1" :step="0.1"></el-slider> -->
                    <br><span>音量</span>
                    <el-select v-model="audioOption.volume" placeholder="Select" class="ms-5 " style="width: 120px">
                        <el-option
                            v-for="item in [{ 'label': '默认', 'value': 'default' }, { 'label': '大声', 'value': 'loud' }, { 'label': '平静', 'value': 'silent' }, { 'label': '中等', 'value': 'medium' }, { 'label': '温柔', 'value': 'soft' }, { 'label': '聒噪', 'value': 'x-loud' }, { 'label': '细语', 'value': 'x-soft' }]"
                            :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                    ...
                    <el-button type="primary" @click="playAudio(audioOption.text)">Play</el-button>
        ...
        <template #footer>
            <div class="dialog-footer d-flex justify-content-end">
                ...
                <el-button @click="audioSettingDialogVisible = false">Cancel</el-button>
                <el-button type="primary" @click="audioSettingDialogVisible = false">
                    Confirm
                </el-button>
            </div>
        </template>
    ...
    <!-- ocr服务对话框 -->
    <el-dialog v-model="ocrSettingDialogVisible" title="OCR服务设置" width="35vw" center>
        <div class="d-flex flex-column justify-content-around align-content-start p-2">
            <el-tabs v-model="ocr.servicefrom" class="demo-tabs">
                <el-tab-pane label="Azure OCR" name="azure" class="p-2">
                    ...
                    <div style="height: 20px;"></div>
                    <el-input v-model="ocr.azure.key" style="width: 340px" placeholder="Please input azure key"
                        clearable type="password" show-password />
                </el-tab-pane>
                <el-tab-pane label="Paddle OCR" name="paddle" class="p-2">
                    <el-text>暂未开放</el-text>
                </el-tab-pane>
            </el-tabs>
        </div>
        <template #footer>
            <div class="dialog-footer">
                <div class="d-flex justify-content-end">
                    <el-button class="" type="primary" @click="ocrSettingDialogVisible = false">
                        处理当前图片
                    </el-button>
                    <el-button @click="ocrSettingDialogVisible = false">取消</el-button>
                    <el-button type="primary" @click="saveOcrSetting">
                        保存
                    </el-button>
                </div>
            </div>
